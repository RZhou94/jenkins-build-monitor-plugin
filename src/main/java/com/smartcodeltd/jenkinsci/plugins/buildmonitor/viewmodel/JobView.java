...
public class JobView {
	...
	private final static Map<Result, String> statuses = new HashMap<Result, String>() {
		{
			...
			put(FAILURE, "failing");
			put(ABORTED, "failing"); // if someone has aborted it then something
										// is clearly not right, right? :)
		}
	};

	public static JobView of(Job<?, ?> job) {
		return new JobView(job, new BuildAugmentor(), RelativeLocation.of(job),
				new Date());
	}
	...
	public String elapsedSinceLastBuild() {
		return formattedDate(lastCompletedBuild().elapsedSinceLastBuild());
	}
	...
	public Set<String> culprits() {
		...
		// todo: consider introducing a BuildResultJudge to keep this logic in
		// one place
	...
	private JobView(Job<?, ?> job, BuildAugmentor augmentor,
			RelativeLocation relative, Date systemTime) {
		this.job = job;
	...
	private BuildViewModel buildViewOf(Run<?, ?> build) {
		...
		return BuildView
				.of(job.getLastBuild(), augmentor, relative, systemTime);
	}
}